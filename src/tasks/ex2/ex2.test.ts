...
describe('ex2.ts', () => {
    ...
    describe('#filter_names()', () => {
        ...
        it('should return the correct output.', () => {
            const names = ['Ben', 'John', 'Jane', 'Colin', 'Jeff', 'Jim', 'Sam', 'Sally', 'Derek', 'Helen', 'Ann', 'Claire', 'Amy', 'Samantha', 'Sarah', 'Mike', 'Fred', 'Molly'];
            const result = filter_names(names);
            expect(result).toEqual(['John', 'Jane', 'Mike', 'Jeff', 'Fred']);
        });
    });
});